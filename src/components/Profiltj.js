...
function Profiltj() {
  ...
  useEffect(() => {
    ...
    var dd = [];
    fetch("http://localhost:8000/api/profilpublic")
      .then((r) => r.json())
      .then((d) => paraqitje(d));
  }, []);
  const shkomajtas = () => {
  ...
  const Click1 = (e) => {
    ...
    fetch("http://localhost:8000/api/clickpublic1", {
      body: data,
      method: "POST",
    })
      .then((r) => r.json())
      .then((d) => {
        if (d["proces"] == "done") {
          window.location.href = d["loaction"];
        console.log("text esgte" + text)
        }
      });
  };
